...
/**
 * Fully replaces one chat's config (label + plans array). Used by the admin
 * dashboard, which sends the complete desired state for a chat on save.
 */
function setChatConfig(chatId, { label, plans }) {
  const all = readAll();
  const key = String(chatId);
  const cleanPlans = (plans || [])
    .filter((p) => p && p.name && p.panelId !== '' && p.panelId !== undefined && p.panelId !== null)
    .map((p) => ({ name: String(p.name).trim(), panelId: Number(p.panelId) }));

  all[key] = { label: label || '', plans: cleanPlans };
  writeAll(all);
  return all[key];
}

/**
 * Removes a chat's config entirely.
 */
